...
namespace SignalRWebUl.Controllers
{
	public class DiscountController : Controller
	{
		...
		public async Task<IActionResult> CreateDiscount(CreateDiscountDto createDiscountDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7217/api/Discount", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			await client.GetAsync($"https://localhost:7217/api/Discount/ChangeStatusToTrue/{id}");
			return RedirectToAction("Index");
		}
		...
		{
			var client = _httpClientFactory.CreateClient();
			await client.GetAsync($"https://localhost:7217/api/Discount/ChangeStatusFalse/{id}");
			return RedirectToAction("Index");
		}
	}
}
